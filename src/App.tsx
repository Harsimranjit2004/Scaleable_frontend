...
const LANGUAGES: Record<number, { label: string; monaco: string; default: string }> = {
  ...
  63: {
    label: "JavaScript",
    monaco: "javascript",
    default: `const n = 10000;\nconst arr = Array.from({length: n}, (_, i) => Math.sin(i * 1.1) * 1000 | 0);\nconsole.log("Array size:", n);\nconst start = Date.now();\narr.sort((a, b) => a - b);\nconsole.log("Min:", arr[0], "Max:", arr[n-1]);\nconsole.log("Sorted in:", Date.now() - start + "ms");\n`,
  },
  50: {
    label: "C",
    monaco: "c",
    default: `#include <stdio.h>\n#include <time.h>\n#define N 300\nint A[N][N], B[N][N], C[N][N];\nint main() {\n    for (int i = 0; i < N; i++)\n        for (int j = 0; j < N; j++) {\n            A[i][j] = (i * 3 + j) % 100;\n            B[i][j] = (i - j * 2) % 100;\n        }\n    clock_t start = clock();\n    for (int i = 0; i < N; i++)\n        for (int j = 0; j < N; j++) {\n            C[i][j] = 0;\n            for (int k = 0; k < N; k++)\n                C[i][j] += A[i][k] * B[k][j];\n        }\n    double ms = (double)(clock() - start) / CLOCKS_PER_SEC * 1000;\n    printf("Matrix %dx%d computed in %.2fms\\n", N, N, ms);\n    printf("C[0][0]=%d C[299][299]=%d\\n", C[0][0], C[299][299]);\n    return 0;\n}\n`,
  },
};
...
export default function App() {
  ...
  function drawChart(hist: MetricHistory[]) {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext("2d");
    if (!ctx) return;
    ...
    const maxQ = Math.max(...hist.map((h) => h.queued), 1);
    const maxW = Math.max(...hist.map((h) => h.workers), 1);
    const pad = { top: 12, right: 16, bottom: 24, left: 32 };
    const chartW = W - pad.left - pad.right;
    const chartH = H - pad.top - pad.bottom;
    ...
    drawLine(hist.map((h) => h.queued), maxQ, "#3b82f6");
    drawLine(hist.map((h) => h.workers), maxW, "#22c55e");

    ctx.fillStyle = "#6b7280";
    ctx.font = "10px monospace";
    ctx.fillText(hist[0].time, pad.left, H - 4);
    ctx.fillText(hist[hist.length - 1].time, W - pad.right - 40, H - 4);
  }
    ...
    try {
      const res = await axios.post(`${API_BASE}/submit/batch`, {
        submissions: [{
          ...
          language_id: langId,
          problem_id: `demo-${Date.now()}`,
          callback_url: `${API_BASE.replace("3000", "3001")}/callback`,
          timeout: 10000,
          memory_limit: 256,
        }],
      });
      pollForResult(res.data.tokens[0]);
    } catch (e) {
      setJobStatus("runtime_error");
      setSubmitting(false);
    }
    ...
    <div style={{ display: "flex", height: "100vh", background: "#0d1117", color: "#e6edf3", fontFamily: "'JetBrains Mono', 'Fira Code', monospace", overflow: "hidden" }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", borderRight: "1px solid #21262d", minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 16px", borderBottom: "1px solid #21262d", background: "#161b22" }}>
          ...
          <div style={{ flex: 1 }} />
          {Object.entries(LANGUAGES).map(([id, lang]) => (
            <button key={id} onClick={() => handleLangChange(Number(id))}
              style={{ padding: "4px 12px", fontSize: 12, borderRadius: 6, border: "1px solid", cursor: "pointer", transition: "all 0.15s",
                borderColor: langId === Number(id) ? "#58a6ff" : "#30363d",
                background: langId === Number(id) ? "#1f3a5f" : "transparent",
                color: langId === Number(id) ? "#58a6ff" : "#8b949e" }}>
              {lang.label}
        ...
        <div style={{ flex: 1, minHeight: 0 }}>
          <Editor height="100%" language={LANGUAGES[langId].monaco} value={code}
        ...
        <div style={{ padding: "10px 16px", borderTop: "1px solid #21262d", background: "#161b22", display: "flex", alignItems: "center", gap: 12 }}>
          <button onClick={handleSubmit} disabled={submitting}
            style={{ padding: "8px 24px", fontSize: 13, fontWeight: 600, borderRadius: 6, border: "1px solid", fontFamily: "inherit", cursor: submitting ? "not-allowed" : "pointer", transition: "all 0.15s",
              background: submitting ? "#1f3a5f" : "#238636", borderColor: submitting ? "#30363d" : "#2ea043", color: submitting ? "#8b949e" : "#ffffff" }}>
            {submitting ? "▶ Running..." : "▶ Run"}
          </button>
          <button onClick={handleStressTest}
            style={{ padding: "8px 16px", fontSize: 13, fontWeight: 600, borderRadius: 6, background: "transparent", border: "1px solid #f59e0b", color: "#f59e0b", cursor: "pointer", fontFamily: "inherit", transition: "all 0.15s" }}>
            ⚡ Stress Test
          </button>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ width: 8, height: 8, borderRadius: "50%", background: statusCol, boxShadow: submitting ? `0 0 8px ${statusCol}` : "none", transition: "all 0.3s" }} />
          ...
          {result?.execution_time && (
            <span style={{ fontSize: 11, color: "#6e7681", marginLeft: "auto" }}>{result.execution_time}ms · exit {result.exit_code}</span>
          )}
        </div>
        <div style={{ height: 180, borderTop: "1px solid #21262d", background: "#0d1117", overflow: "auto", padding: "12px 16px" }}>
          <div style={{ fontSize: 11, color: "#6e7681", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.08em" }}>Output</div>
          {result ? (
            ...
            <span style={{ fontSize: 13, color: "#6e7681" }}>{jobStatus === "pending" ? "Executing..." : "Run your code to see output"}</span>
          )}
        </div>
      </div>
      <div style={{ width: 320, display: "flex", flexDirection: "column", background: "#161b22", overflow: "auto" }}>
        ...
        <div style={{ padding: "0 16px 12px" }}>
          ...
          <div style={{ background: "#0d1117", borderRadius: 8, border: "1px solid #21262d", padding: 12, fontSize: 11, color: "#8b949e", lineHeight: 1.8 }}>
            {[
              { icon: "🌐", label: "React Frontend", color: "#58a6ff" },
              { icon: "↓", label: "", color: "#30363d" },
              { icon: "⚡", label: "Express API  :3000", color: "#a78bfa" },
              { icon: "↓", label: "", color: "#30363d" },
              { icon: "📦", label: "Redis Queue", color: "#f59e0b" },
              { icon: "↓", label: "", color: "#30363d" },
              { icon: "⚙️", label: `Workers ×${metrics.workerCount}  (auto-scale)`, color: "#22c55e" },
        ...
        <div style={{ padding: "0 16px 12px" }}>
          <div style={{ fontSize: 10, color: "#6e7681", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8 }}>Deployment</div>
          <div style={{ background: "#0d1117", borderRadius: 8, border: "1px solid #21262d", padding: 12 }}>
            {[
              { label: "API", value: "Oracle Cloud VM" },
              { label: "IP", value: "141.148.79.21" },
              { label: "Workers", value: "Docker Compose" },
              { label: "Local K8s", value: "docker-desktop" },
              { label: "Min pods", value: "1" },
              { label: "Max pods", value: "10" },
            ].map((row) => (
              <div key={row.label} style={{ display: "flex", justifyContent: "space-between", padding: "3px 0", borderBottom: "1px solid #21262d", fontSize: 11 }}>
